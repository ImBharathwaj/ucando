import feature.CustomerSpendingFeatures
...
object CustomerIntelligenceJob {
    def main(args: Array[String]): Unit = {
        val spark = SparkSession
            .builder()
            .appName("CustomerIntelligenceJob")
            .master("local[*]")
            .getOrCreate()
    ...
    val customerFeaturesDf =
      CustomerSpendingFeatures.build(
        transformedDf
      )
    ...
    val query =
      customerFeaturesDf
        ...
        .option(
          "checkpointLocation",
          "/tmp/ucando/checkpoints/customer-intelligence-v2"
        )
        .start()
        .awaitTermination()
    }
