...
from {{ ref('customer_intelligence') }} ci
left join {{ source('public', 'customer_segments') }} seg
on ci.customer_id = seg.customer_id
left join {{ source('public', 'customer_loan_propensity') }} lp1
on ci.customer_id = lp1.customer_id
left join {{ source('public', 'customer_loan_propensity_v2') }} lp2
on ci.customer_id = lp2.customer_id
left join {{ source('public', 'customer_graph_features') }} gf
